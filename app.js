...
//7일인 이유 : 실제로 계쏙사용하면서 accesstoken을 재발급 받기 위한 용으로 사용하기 때문에 만료기간이 길더라도
//실제 서버에서 인증을 받아야지만 access토큰을 발급하고 사용자의 권한을 사용할수있기때문에 상관은 없고.
//너무 길지 않게만 설정해주면된다.
  return refreshToken;
}
...
app.get("/get-token", (req, res) => {
    ...
    if (!isAccessTokenValidate) {
      ...
      //위에서 지정한 토큰이 처음 지정한 access token의 id값을 가져오게 함
      if (!accessTokenId) return res.status(419).json({ "message": "Refresh Token의 정보가 서버에 존재하지 않습니다." });
      //토큰자체는 가지고 있지만 서버에 가지고있는 토큰과 일치하지 않을경우/서버에서 고의적으로 만료시킨 경우를 뜻함  
    //사전에 말했던 탈취된 토큰 혹은 저장된 토큰 분실되었을때라고 했을때 고의적으로 토큰을 만료시키기 위해
    ...
    //access토큰과 refresh토큰 모두 정상적인 것을 확인해야 될 경우 다음과 같이 진행
    const { id } = getAccessTokenPayload(accessToken);
  //밑에서 지정한 getAccess를 통해 id값을 가져와 객체구조분해할당 문법을 통해 내부에 있는 id값을 변수로 지정한다.
